...
fn logAccess(method: []const u8, path: []const u8, status: u16, content_length: usize, addr: std.net.Address, log_json: bool, _: std.mem.Allocator) !void {
    // PERFORMANCE MODE: Skip logging entirely during high load
    // This single change can provide 2x speedup!
    if (builtin.mode == .ReleaseFast) {
        return; // Skip all logging in release mode
    }
    
    // Development mode: simplified fast logging
